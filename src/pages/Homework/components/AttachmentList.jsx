...
import { formatFileSize, getFileIconClass, isPreviewable } from '../../../utils/helpers';
...
const AttachmentList = ({ attachments }) => {
    ...
    const handleFileAction = async (path, action) => {
        try {
            const res = await attachApi.get('/get/downloadId', { params: { path } });
            const token = res.data.data;
            const baseUrl = attachApi.defaults.baseURL;

            if (action === 'preview') {
                const previewUrl = `${baseUrl}/download?mode=inline&path=${encodeURIComponent(path)}&token=${token}`;
                window.open(previewUrl, '_blank');
            } else if (action === 'download') {
                const downloadUrl = `${baseUrl}/download?path=${encodeURIComponent(path)}&token=${token}`;
                ...
                {attachments.map(att => (
                    <li key={att.filePath} className={styles.attachmentItem}>
                        <div className={styles.attachmentInfo}>
                            <i className={getFileIconClass(att.fileName)}></i>
