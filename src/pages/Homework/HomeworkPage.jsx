...
const HomeworkPage = () => {
    ...
    const navigateTo = useCallback((viewName, viewData = null) => {
        ...
        else if (viewName === 'edit') { /* 编辑逻辑通常在组件内处理或带ID */ }

        // 区分：学生看作业(homework) vs 老师看提交列表(submissions) vs 老师批改(submission)
        else if (viewName === 'homeworkDetail') hash = `#/homework/${viewData}`; // 学生查看作业
        else if (viewName === 'submissionList') hash = `#/homework/${viewData}/submissions`; // 老师查看某作业的提交列表
        else if (viewName === 'submissionGrading') hash = `#/submission/${viewData}`; // 老师批改某具体提交
    ...
    useEffect(() => {
        const handleHashChange = () => {
            ...
            else if (parts[0] === 'submission' && parts[1] && parts[1] !== 'undefined') {
    ...
    const handleOpenDiscussion = useCallback((target) => setDiscussionTarget(target), []);
    ...
    return (
        <div className={styles.appContainer}>
            ...
            <DiscussionDrawer target={discussionTarget} onClose={() => setDiscussionTarget(null)} />
        </div>
    );
};
